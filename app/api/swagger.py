...
def validate(data, schema_id, code=400, *args, **kwargs):
  ...
  return flasgger.validate(data, schema_id, apispec, validation_error_handler=error_handler, *args, **kwargs)
...
def init_app(app, title='API', version='1.x.x', spec='apispec.yaml'):
  ...
  swagger = flasgger.Swagger(app, template=template)
  global _validate
  _validate = swagger.validate
  return swagger